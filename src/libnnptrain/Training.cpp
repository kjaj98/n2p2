...
void Training::selectSets()
{
    ...
    // new
    bool bExactTestSelection = settings.keywordExists("test_fraction_exact");
    vector<char> testSet(numStructures, 0);
    if(bExactTestSelection) 
    {
        if(myRank == 0)
	    {	
            ...
            for(size_t i = 0; i < numStructures; ++i)	    
                indices.push_back(i);
            size_t nTestSet = static_cast<size_t>(round(testSetFraction * numStructures));
            for(size_t i = 0; i < nTestSet; ++i)
            {
                size_t sel = gsl_rng_uniform_int(rng, indices.size());
                testSet[indices[sel]] = 1;
                indices.erase(next(indices.begin(), sel));
            }	
        }

        MPI_Bcast(testSet.data(), numStructures, MPI_BYTE, 0, comm);
    }
